...
//    打开多媒体文件
    ...
    av_log(&extAudCtx, AV_LOG_VERBOSE, "src media path=%s \n", pFmtCtx->url);
//    打印流数量
    av_log(&extAudCtx, AV_LOG_VERBOSE, "src media stream number=%d \n", pFmtCtx->nb_streams);
//    打印码率
    av_log(&extAudCtx, AV_LOG_VERBOSE, "src media average ratio:%lld kbps\n", (int64_t) (pFmtCtx->bit_rate / 1024));
...
//    ctx->duration的单位是微妙，先转为秒
    ...
    sec = duration_sec % 60;
    av_log(&extAudCtx, AV_LOG_VERBOSE, "media total time: %d:%d:%d \n", hour, minute, sec);
...
//    2, 从多媒体中找到音频流
    ...
    av_log(&extAudCtx, AV_LOG_INFO, "av_find_best_stream AUDIO: %d\n", aud_idx);
//    3, 打开目的文件的上下文
